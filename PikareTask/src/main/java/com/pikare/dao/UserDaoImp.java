...
public class UserDaoImp implements UserDao {
	...
	public List<Users> getByUserRoles() {
		...
		try {  
		    session = sessionFactory.openSession();  
		    txn = session.beginTransaction();
		    Query query = session.createQuery(" from Users as item join item.userRole as roles where roles.role= 'ROLE_USER' and item.enabled = 1 order by name"); //You will get Weayher object
		    users = (List<Users>)query.list();
		    txn.commit();
		    System.out.println("başarli bir şekilde eklendi");
